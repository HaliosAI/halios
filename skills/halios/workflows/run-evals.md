...
On HTTP 402 `usage_limit_exceeded`, stop automated retries and report the server-provided meter,
usage, remediation, alternatives, and billing URL. BYOK is an alternative only for
`managed_ai_tokens`; it does not bypass `check_results` or `trace_ingest_bytes` limits. If a run is
marked `incomplete_quota` without structured 402 detail, report the run ID and advise enabling
pay-as-you-go or waiting for the monthly reset. Do not treat quota exhaustion as an agent code bug.
