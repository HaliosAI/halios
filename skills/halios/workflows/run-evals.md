...
7. On a 402 Usage Limit Exceeded error (e.g. `Halios usage allowance exhausted: Monthly managed ai tokens allowance is exhausted`),
   recognize that the organization's monthly allowance has been reached. Inform the user with the direct billing URL
   to enable pay-as-you-go (`/settings/billing`) or switch to BYOK custom models. Do not retry or treat quota exhaustion as an agent code bug.
